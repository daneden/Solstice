...
struct SolsticeTimelineProvider: AppIntentTimelineProvider {
	...
	func timeline(for configuration: SolsticeConfigurationIntent, in context: Context) async -> Timeline<SolsticeWidgetTimelineEntry> {
		...
		guard let coordinate = widgetLocation?.coordinate,
		      let todaySolar = NTSolar(for: currentDate, coordinate: coordinate, timeZone: widgetLocation?.timeZone ?? .autoupdatingCurrent)
		else {
			return Timeline(
				entries: [
		...
		#if os(watchOS)
			// watchOS widget complications are always considered "viewed" when installed on a watch face.
			// To prevent timeline generation from occurring too frequently and exhausting generation budgets,
			// we lower the number of total entries generated by only generating enough for one day.
		...
		for dayOffset in 0 ..< daysToGenerate {
			guard let dayDate = calendar.date(byAdding: .day, value: dayOffset, to: today),
			      let daySolar = NTSolar(for: dayDate, coordinate: coordinate, timeZone: widgetLocation?.timeZone ?? .autoupdatingCurrent)
			else {
				continue
			}
		...
		for date in allKeyTimes {
			mergedEntries.append(makeEntry(at: date))
		}
		for date in filteredHourlyTimes {
			mergedEntries.append(makeEntry(at: date))
		}
	...
	func placeholder(in _: Context) -> SolsticeWidgetTimelineEntry {
