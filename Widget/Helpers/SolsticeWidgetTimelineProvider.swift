...
struct SolsticeTimelineProvider: AppIntentTimelineProvider {
	...
	/// Fetches current location and reverse geocodes it to a placemark
	private func fetchCurrentLocation() async -> (placemark: CLPlacemark?, error: LocationError?) {
		guard SolsticeWidgetLocationManager.isAuthorized else {
			return (nil, .notAuthorized)
		}

		guard let location = await SolsticeWidgetLocationManager.shared.getLocation() else {
			return (nil, .locationUpdateFailed)
		}

		guard let placemark = try? await geocoder.reverseGeocodeLocation(location).first else {
			return (nil, .reverseGeocodingFailed)
		}

		return (placemark, nil)
	}

	/// Resolves the widget location from configuration
	private func resolveWidgetLocation(for configuration: SolsticeWidgetConfigurationIntent) async -> (location: SolsticeWidgetLocation?, error: LocationError?) {
		switch configuration.locationType {
		case .customLocation:
			// Custom location: use the CLPlacemark directly
			if let placemark = configuration.customLocation,
			   let widgetLocation = placemark.widgetLocation {
				return (widgetLocation, nil)
			}
			return (nil, .locationUpdateFailed)

		case .currentLocation:
			// Current location: fetch and reverse geocode
			let (placemark, error) = await fetchCurrentLocation()
			let widgetLocation = getLocation(for: placemark, isRealLocation: true)
			return (widgetLocation, error)
		}
	}

	func snapshot(for configuration: SolsticeWidgetConfigurationIntent, in context: Context) async -> SolsticeWidgetTimelineEntry {
		let (widgetLocation, error) = await resolveWidgetLocation(for: configuration)
		let resolvedLocation = widgetLocation ?? (context.isPreview ? .proxiedToTimeZone : nil)

		return SolsticeWidgetTimelineEntry(
			date: Date(),
	...
	func timeline(for configuration: SolsticeWidgetConfigurationIntent, in context: Context) async -> Timeline<SolsticeWidgetTimelineEntry> {
		let (widgetLocation, error) = await resolveWidgetLocation(for: configuration)
		...
		// Generate entries for multiple days (reduced on watchOS for performance)
		#if os(watchOS)
